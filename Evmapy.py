...
class Evmapy():
    ...
    def __prepare(system, emulator, core, rom, playersControllers, guns):
        # ===== SOLUÇÃO DEFINITIVA: PULAR EMULADORES STANDALONE =====
        # Emuladores standalone (não-RetroArch) matam o processo pai
        # e retornam 255, o que confunde o Evmapy e impede o jogo de iniciar
        # 
        # Lista completa de emuladores que são standalone:
        # - Sistemas Windows
        # - Emuladores de consoles (Dolphin, PCSX2, etc)
        # - Ports de jogos nativos
        # - Emuladores de computadores antigos
        # - Qualquer emulador que não seja RetroArch
        ...
        standalone_emulators = [
            ...
            'triforce',  # Arcade (GameCube based)
            
            # Emuladores de consoles (Sony)
            'pcsx2', 'pcsx2-64',  # PS2
            'rpcs3',  # PS3
            'ppsspp',  # PSP
            'duckstation',  # PS1
            ...
            # Emuladores de consoles (Microsoft)
            'xemu',  # Xbox
            'xenia',  # Xbox 360
            
            # Emuladores de consoles (Atari)
            'stella',  # Atari 2600
            'prosystem',  # Atari 7800
            'virtualjaguar',  # Atari Jaguar
            'atari800',  # Atari 800
            
            # Emuladores de consoles (Outros)
            'openmsx',  # MSX
            'o2em',  # Odyssey2
            ...
            'eduke32',  # Duke Nukem
            'ioquake3',  # Quake 3
            'openarena',  # Quake 3 based
            'smw',  # Super Mario War
            'vvvvvv',  # VVVVVV
            'cave_story',  # Cave Story (NXEngine)
            'devilutionx',  # Diablo
            ...
            'sonic',  # Sonic CD, Sonic 1/2 (RSDK)
            'cannonball',  # OutRun engine
            'sm64',  # Super Mario 64 PC port
            'srb2',  # Sonic Robo Blast 2
            'reminiscence',  # Flashback
            'rigelengine',  # Duke Nukem II
            ...
            'nblood',  # Blood
            'rednukem',  # Duke Nukem
            'raze',  # Build engine games
            'eternity',  # Doom engine
            'prboom',  # Doom engine
            'zdoom',  # Doom engine
            
            # Emuladores de computadores antigos
            'scummvm',  # Point & click games
            'dosbox', 'dosbox_staging', 'dosboxx',  # DOS
            'fsuae',  # Amiga
            'amiberry',  # Amiga
            'hatari',  # Atari ST
            'vice',  # C64
            ...
            # Emuladores de arcade/Daphne
            'hypseus-singe',  # Daphne (laserdisc games)
            'daphne',  # Daphne
            'singe',  # Singe
            'hikaru',  # Sega Hikaru
            'gaelco',  # Gaelco
            'cave3rd',  # Cave 3rd
            'namco2x6',  # Namco System 2x6
            
            # Outros sistemas
            'openlara',  # Tomb Raider engine
            'openomf',  # One Must Fall
            ...
            'abuse',  # Abuse
            'julius',  # Caesar III
            'augustus',  # Caesar III mod
            'lgeneral',  # Panzer General
            'freespace2',  # Freespace 2
            'fs2open',  # Freespace 2 Open
            'wxlauncher',  # Freespace 2 launcher
            'sc2',  # Star Control 2 (Ur-Quan Masters)
            'uqm',  # Ur-Quan Masters
            'naev',  # NAEV
            'endless-sky',  # Endless Sky
            'vegastrike',  # Vega Strike
            'olive',  # Olive
            'pioneer',  # Pioneer space sim
            'oolite',  # Oolite (Elite clone)
            'torcs',  # TORCS racing
            'stuntrally',  # Stunt Rally
        ...
        # Se for Windows ou qualquer emulador standalone, pula Evmapy
        if system in standalone_emulators or emulator in standalone_emulators:
            eslog.debug("=" * 60)
            eslog.debug(f"EMULADOR STANDALONE DETECTADO: {system}/{emulator}")
            eslog.debug("Pulando Evmapy para evitar erro 255")
            eslog.debug("Este emulador mata o processo pai (código 255 é normal)")
            eslog.debug("=" * 60)
            
            # ===== SOLUÇÃO PARA TELA PRETA =====
            # Força a liberação do display antes do jogo iniciar
            try:
                # Suspende o ES
                subprocess.call(["batocera-es-swissknife", "--suspend"])
                
                # Dá um tempo para o sistema se organizar
                time.sleep(1)
                
                # Força uma troca de contexto no X11
                os.system("xset dpms force off > /dev/null 2>&1")
                os.system("xset dpms force on > /dev/null 2>&1")
                os.system("xrefresh -display :0 > /dev/null 2>&1")
                
                eslog.debug("Display liberado para o emulador standalone")
            except Exception as e:
                eslog.debug(f"Erro ao liberar display: {e}")
